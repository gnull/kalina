...
drawMenu s =
    if s ^. displayHelp then
      ...
      case s ^. menuState of
        MenuFeeds z -> g $ renderList renderFeed True $ z ^. listState
        MenuItems False is -> g $ renderList renderItem True $ is ^. liItems ^. listState
...
handleMenu _ st e = continue =<< menuState f st
  where
    f s = case s of
      MenuFeeds z -> MenuFeeds <$> listState (handleListEventVi handleListEvent e) z
      MenuItems False i -> MenuItems False <$> (liItems . listState) (handleListEventVi handleListEvent e) i
